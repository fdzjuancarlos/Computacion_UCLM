...
resulutor(X, L):- recorrer(X, L).

recorrer(X, []):- false.
recorrer(X, [C|R]):- resolutor(X,Y,L); resolutor(Y,X,L); recorrer(X,R).

resolutor(X, not(X), L):- write(encontrado).
resolutor(not(X), X, L):- write(encontrado).
...
resolutor((X;Y), (not(X);Z)):- repeticion(Y, Z, C), (clausula(C); esta(C, L)), !, fail; resolutor(C, [L|C]).
resolutor((X;Y), (Z;not(X))):- repeticion(Y, Z, C), (clausula(C); esta(C, L)), !, fail; resolutor(C, [L|C]).
